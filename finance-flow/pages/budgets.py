...
from database.crud import get_budgets, set_budget, delete_budget
...
def _render_budget_overview(month, year):
    st.subheader(
        t("subheader_budget_overview", date=datetime(year, month, 1).strftime("%B %Y"))
    )
    ...
    for r in results:
        ...
        with col1:
            st.markdown(
                f'<span class="budget-category">{r["category"]}</span>',
                unsafe_allow_html=True,
            )
        with col2:
            st.markdown(
                f'<span class="budget-spend">${r["actual"]:,.2f} / ${r["limit"]:,.2f}</span>',
                unsafe_allow_html=True,
            )
        with col3:
            if pct >= 100:
                st.markdown(
                    f'<span class="budget-alert-danger">{pct}%</span>',
                    unsafe_allow_html=True,
                )
            elif pct >= threshold:
                st.markdown(
                    f'<span class="budget-alert-warning">{pct}%</span>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f'<span style="color:#94a3b8;font-weight:600;font-size:0.9rem;">{pct}%</span>',
                    unsafe_allow_html=True,
                )
        with col4:
            ...
            if remaining < 0:
                st.markdown(
                    f'<span class="budget-alert-danger">${abs(remaining):,.2f} {t("label_over_suffix")}</span>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f'<span style="color:#94a3b8;font-size:0.85rem;">${remaining:,.2f} {t("label_remaining_suffix")}</span>',
                    unsafe_allow_html=True,
                )
    ...
    with st.form("budget_form"):
        ...
        with col1:
            category = st.text_input(
                t("form_category_name"), placeholder=t("placeholder_category_name")
            )
            limit_amount = st.number_input(
                t("form_monthly_limit"), min_value=0.01, format="%.2f"
            )
        with col2:
            sel_month = st.selectbox(t("form_month"), range(1, 13), index=month - 1)
            sel_year = st.number_input(
                t("form_year"), min_value=2020, max_value=2100, value=year
            )

        if st.form_submit_button(
            t("btn_save_budget"), type="primary", use_container_width=True
        ):
            if category and limit_amount > 0:
                set_budget(category.strip(), sel_month, sel_year, limit_amount)
                st.success(
                    t("success_budget_saved", category=category, limit=limit_amount)
                )
                st.rerun()
            else:
