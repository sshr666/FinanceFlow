...
def get_db_path():
    ...
    if resolved.exists():
        logger.warning(
            "DB not writable at: %s (parent writable=%s, file writable=%s)",
            resolved,
            _is_writable(parent),
            _is_writable(resolved),
        )
    else:
        logger.warning(
            "DB parent not writable at: %s (parent writable=%s)",
            parent,
            _is_writable(parent),
        )
...
def init_db():
    ...
    if not writable:
        logger.critical(
            "Database is NOT WRITABLE: %s. All write operations will fail!", err
        )
